...
//with async await
async function main() {
    const getCandyResponse = await goGetCandies();
    const passCandies = await sellCandies(getCandyResponse);
    console.log(passCandies);
}

// main();

//with vanilla js
goGetCandies().then((val) => {
    return val;
}).then((val) => {
    let calculation = sellCandies(val);
    return calculation;
}).then((val) => {
    console.log(val);
})